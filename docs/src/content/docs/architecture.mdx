...
<Aside type="caution" title="Current implementation and target framework">
This page still documents the current S2S-oriented implementation. The confirmed target architecture is Harness-neutral and plugin-composed: a trusted VoiceClaw Kernel coordinates Relay, Desktop Host, Clients, and Harnesses; VoiceClaw Plugin Packages deliver Feature Plugins made from runtime Contributions behind versioned Capability Contracts. Current delivery prioritizes a Desktop prototype through Kernel Phase 0, Desktop Host, provider-neutral Routing, and a real Codex app-server vertical slice; Archive and Memory remain official Feature Plugins on a deferred path and do not block non-persistent Harness execution. The normative engineering boundary is maintained in `docs/architecture/voiceclaw-harness-boundary.md` while this public page is migrated.
</Aside>

## Target Framework Direction

VoiceClaw is evolving into a composable Agent interaction and capability-hosting framework for multiple Harnesses and devices. Codex, Claude Code, DeepSeek Harness, and other Harnesses remain peers; no one Harness supplies VoiceClaw's Kernel.

```mermaid
flowchart TB
    Kernel["Trusted VoiceClaw Kernel<br/>lifecycle · grants · RPC · isolation · audit · fencing"]
    Package["VoiceClaw Plugin Package"]
    Feature["Feature Plugin<br/>user-managed capability"]
    RelayContribution["relay-service Contribution"]
    DesktopContribution["desktop-service / worker Contribution"]
    ClientContribution["client-ui Contribution"]
    ProviderContribution["provider-integration Contribution"]
    HarnessExtension["harness-extension Contribution"]
    HarnessPlugin["External Harness Plugin"]

    Kernel --> Package --> Feature
    Feature --> RelayContribution
    Feature --> DesktopContribution
    Feature --> ClientContribution
    Feature --> ProviderContribution
    Feature --> HarnessExtension --> HarnessPlugin
```

Feature Plugin composes, Contribution implements, and Capability Contract decouples. The default distribution will provide Archive and Memory as official Feature Plugins. Their implementations are replaceable, while identity, authorization, deletion, audit, ownership, isolation, migration coordination, and generation fencing remain non-bypassable Kernel responsibilities.
...
2. Relay session cleanup runs:
   ...
   - Currently syncs conversation transcript to the brain agent through a legacy cleanup path; the target architecture removes this behavior and never treats it as a fallback for the Memory Feature Plugin
...
- The current implementation syncs the transcript to the brain on disconnect. This is a documented migration gap: the target architecture removes it and permits Memory production only through the separately authorized Memory Feature Plugin.
