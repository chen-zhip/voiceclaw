...
## STT/TTS composed architecture

`mode: "stt-tts"` selects a `ComposedAdapter` without changing the existing S2S adapters. The composed adapter orchestrates replaceable provider ports, while `OutputRouter` owns the separation of private thinking, spoken summaries, and detailed screen text.
...
flowchart LR
    ...
    STT -->|final transcript| Harness[HarnessAdapter]
...
See the repository's `docs/harness-integration.md` for Harness IDs and capability behavior, and `docs/stt-tts-migration.md` for opt-in migration from S2S.
