...
describe('executeStandaloneTool', () => {
  ...
  it('exposes read/write/edit/bash + web_search via STANDALONE_TOOL_NAMES', () => {
    expect([...STANDALONE_TOOL_NAMES].sort()).toEqual([
      'bash',
      'edit',
      'read',
      'web_search',
  ...
  it('web_search returns ok=false on an empty query', async () => {
    const outcome = await executeStandaloneTool('web_search', JSON.stringify({ query: '   ' }), {
  ...
  it('runs read and returns a JSON string with line-numbered content', async () => {
    ...
    const outcome = await executeStandaloneTool('read', JSON.stringify({ path: 'hello.txt' }))
    expect(outcome.ok).toBe(true)
    if (!outcome.ok) throw new Error(outcome.error)
  ...
  posixIt('runs bash, returns stdout, and streams onProgress textDeltas', async () => {
  ...
  posixIt('bash aborts when the external signal fires', async () => {
    const controller = new AbortController()
    const promise = executeStandaloneTool('bash', JSON.stringify({ command: 'sleep 5' }), {
  ...
  it('read supports reading files outside the workspace (read is unrestricted)', async () => {
    ...
    const outcome = await executeStandaloneTool('read', JSON.stringify({ path: outside }))
    expect(outcome.ok).toBe(true)
    if (!outcome.ok) throw new Error(outcome.error)
  ...
  it('durationMs is reported on success and on failure', async () => {
    ...
    const fail = await executeStandaloneTool(
      'read',
      JSON.stringify({ path: join(tmpRoot, 'missing') })
    )
