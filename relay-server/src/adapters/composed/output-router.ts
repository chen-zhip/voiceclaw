...
export class OutputRouter {
  ...
  constructor(private readonly dependencies: OutputRouterDependencies) {
    const requested = dependencies.sentenceBatchSize ?? 1
    this.sentenceBatchSize = Number.isFinite(requested)
  ...
  private async routeSpeech(chunk: Extract<OutputChunk, { type: 'speech.delta' }>): Promise<void> {
    if (chunk.content.length > 500) {
      this.warn(
        `Speech content exceeds the 500 character listening limit (${chunk.content.length})`
      )
    }
    if (this.containsDetailedStructure(chunk.content)) {
      this.warn('structured table or code content was excluded from speech output')
      return
    }
    ...
    this.speechCharacters += chunk.content.length
