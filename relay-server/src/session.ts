...
import type { ClientEvent, SessionConfigEvent, RelayEvent } from './types.js'
import type { ProviderAdapter, SendToClient } from './adapters/types.js'
import { createAdapter, type AdapterFactoryDependencies } from './adapters/index.js'
import {
  effectiveVoiceMode,
  executeSyncTool,
  findRelayTool,
  getGeminiTools,
  getRelayTools,
  isBlockingLatencyClass,
  resolveTavilyKey,
} from './tools/index.js'
import { logAgentBackendSelection } from './agents.js'
import { noteSupervisorSelected } from './supervisor.js'
import { resolveAgentBackend, resolveSessionMode, resolveVoiceMode } from './types.js'
...
export interface ThinkingDataManager {
...
export class RelaySession {
  ...
  private handleRelayEvent(event: RelayEvent) {
    ...
    switch (event.type) {
      case 'turn.started':
        this.tracer.startTurn(event.turnId)
        this.startMediaTurn()
        break
  ...
  private runBlockingAskBrain(callId: string, args: string) {
    ...
    const gatewayUrl =
      process.env.BRAIN_GATEWAY_URL || process.env.OPENCLAW_GATEWAY_URL || 'http://localhost:18789'
    const authToken =
      process.env.BRAIN_GATEWAY_AUTH_TOKEN ||
    ...
    context
      ...
      .then((result) => {
        ...
        if (brainErrorMsg !== null) {
          logError(
            `[session:${this.id}] ask_brain (blocking) returned error payload:`,
            brainErrorMsg
          )
  ...
  private async runEditTool(callId: string, args: string) {
    ...
    log(
      `[session:${this.id}] edit → ${path.slice(0, 120)} (old=${oldString.length}c, new=${newString.length}c${replaceAll ? ', replace_all' : ''})`
    )
  ...
  private async handleMessage(raw: unknown) {
    ...
    switch (event.type) {
      ...
      case 'session.prep':
        ...
        break
      case 'thinking.delete':
        await this.thinkingData.deleteSession(event.sessionId)
        break
      case 'thinking.wipe':
        await this.thinkingData.wipe()
        break
      default:
  ...
  private async handleSessionPrep(config: SessionConfigEvent) {
    try {
      ...
      logError(
        `[session:${this.id}] session.prep ensureWorkspace failed:`,
  ...
  private async handleMintToken(provider: string, model?: string) {
    ...
    if (result.warning) {
      ...
      log(
        `[session:${this.id}] mint_token: minted ephemeral=${result.ephemeral} expiresAt=${result.expiresAt}`
      )
    }
  ...
  private async handleSessionConfig(config: SessionConfigEvent) {
    config = {
      ...config,
      mode: resolveSessionMode(config.mode),
      sessionKey: config.sessionKey ?? this.id,
    }
    ...
    try {
      ...
      logError(
        `[session:${this.id}] ensureWorkspace failed:`,
        err instanceof Error ? err.message : err
      )
    }
      ...
      this.adapter = this.adapterFactory(config, {
        ...
        attachThinking: (thinking, turnId) =>
          this.tracer.attachThinking(thinking, turnId, [
            ...
            config.ttsConfig,
            config.sttProvider?.toLowerCase() === 'deepgram'
              ? { apiKey: process.env.DEEPGRAM_API_KEY }
              : undefined,
            config.ttsProvider?.toLowerCase() === 'elevenlabs'
              ? { apiKey: process.env.ELEVENLABS_API_KEY }
              : undefined,
          ]),
        getThinkingTracePath: (turnId) => this.tracer.getTracePath(turnId),
      })
      await this.adapter.connect(config, sendToClient)
      const preamble = this.adapter.getResumePreamble?.() ?? ''
      const resumeHistory = this.adapter.getResumeHistory?.() ?? []
      this.tracer.setSessionPreamble(preamble || null)
      this.tracer.setResumeHistory(resumeHistory)
  ...
  private async finalizeMediaTurn(turnId: string | null) {
    ...
    try {
      ...
      logError(
        `[session:${this.id}] media finalize failed:`,
        err instanceof Error ? err.message : err
      )
    }
  }
  ...
  private async finalizeSessionMedia() {
    ...
    try {
      ...
      logError(
        `[session:${this.id}] session media finalize failed:`,
        err instanceof Error ? err.message : err
      )
    }
  }
  ...
  private syncTranscriptToBrain() {
    ...
    log(
      `[session:${sessionId}] Syncing transcript to brain (${transcript.length} turns, ${durationMin}min)`
    )
  ...
  for (let attempt = 1; attempt <= TRANSCRIPT_SYNC_BACKOFF_MS.length + 1; attempt++) {
    try {
      ...
      if (backoff === undefined) {
        logError(
          `[session:${sessionId}] Transcript sync gave up after ${attempt} attempts: ${message}`
        )
        throw err
      }
      logError(
        `[session:${sessionId}] Transcript sync attempt ${attempt} failed (${message}), retrying in ${backoff}ms`
      )
